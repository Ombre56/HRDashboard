...
import { NotAcceptedCandidates } from './NotAcceptedCandidates';
...
interface ChildProps{
  acceptedViewData: boolean;
}

export const AcceptedCandidates = ({acceptedViewData}: ChildProps) => {
  const [candidatesData, setCandidatesData] = useState(data);
  return (
    <>
      {acceptedViewData ? candidatesData.map(({ id, name, rating, stages, applied_role, application_date, attachments, accepted }: dataTypes) =>
        accepted &&
